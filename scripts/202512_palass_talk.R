...
dir_plots <- file.path("plots", "palass2025")
...
data_event_biosphere <- infile %>% 
  openxlsx2::wb_to_df(
    ., 
    sheet = "table_4_event_biosphere_changes", 
    start_row = 1, 
    cols = c(1:9),
    skip_empty_rows = TRUE, 
    skip_empty_cols = TRUE
...
data_to_plot <- data_event_durations %>%
  dplyr::right_join(
    ...
    by = join_by(interval_name, interval_abbreviation, event_duration_est_yr)
  ) %>%
  dplyr::mutate(
    across(
      c(change_value, 
...
ggsave(
  filename = file.path(dir_plots, "rates_of_change_ordered.png"),
